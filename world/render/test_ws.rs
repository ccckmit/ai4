//! world/render/test_ws.rs
//! CartPole-v1 closed-form controller with browser-based rendering.
//!
//! Usage:
//!   cargo run --bin world_cartpole_render_test

use ai4::{CartPoleEnv, Env};
use std::thread;
use std::time::Duration;

fn main() {
    // Start the render server (spawns background thread, opens browser)
    let server = ai4::world::render::server::RenderServer::start(8080);

    // Give the server + browser a moment to connect
